...
export function LogoSlideSection() {
    ...
    const Card = ({ src, w, h }: { src: string; w: number; h: number }) => (
        <div className="flex h-[150px] w-[320px] shrink-0 items-center justify-center border border-[#2A2A2A] bg-[radial-gradient(circle_at_center,#000_0%,#171717_50%,#ffffff10_100%)] shadow-[0_0_20px_#00000030] transition-all duration-300 hover:border-[#EFA12D] hover:shadow-[#EFA12D]/20">
            <Image
                src={src}
                alt={t("logoAlt")}
                width={w}
    ...
    return (
        <section className="relative w-full overflow-hidden bg-linear-to-b from-[#0D0D0D] to-[#1A1A1A]">
            <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_center,#000_0%,#0D0D0D_80%,#1A1A1A_100%)] opacity-30" />
