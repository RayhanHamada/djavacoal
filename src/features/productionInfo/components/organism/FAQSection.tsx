...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="rounded-md overflow-hidden border border-[#4A4A4A] transition-all duration-300">
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className={`w-full flex items-center justify-between text-left px-4 py-3 transition-colors duration-300 ${
      ...
      {/* Jawaban */}
      {open && (
        <div className="bg-[#35362D] px-4 pb-4 pt-2 text-[#EAEAEA] text-sm md:text-base leading-relaxed border-t border-[#2B2B2B]">
